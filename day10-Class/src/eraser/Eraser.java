...
public class Eraser {
	 ...
	 */
	Eraser(){
		//TODO 멤버변수인 serial을 count 클래스 변수를 증가시킨
		//     값으로 저장한다.
		this.serial = ++count;
	}
	// (2)매개변수가 있는 생성자
	 ...
	 */
	Eraser(double width){
		// 57라인의 기본생성자 호출 : 매개변수가 비어있는 this()이므로
		this(); 
...
//		}
	...
	// (3) 상태 호출 print()
	public void print() {
		System.out.printf("번호 :%d 현재 지우개 상태는 가로 :%4.2f 세로 :%4.2f 높이 :%4.2f 남은 겹 :%d 색상 :%s%n",serial, width, height, depth, layer, color);
	}
